...
let page: Html.doc = ....
```
In the previous examples, we used the textual implementation of tyxml (module `Tyxml`) in the `tyxml` library), but it works equally well with other implementations:

- The DOM tree with the `js_of_ocaml-tyxml` library. Either the `Js_of_ocaml_tyxml.Tyxml_js` or the reactive version in `Js_of_ocaml_tyxml.Tyxml_js.R`.
