...
The type variables in [`elt`](./Svg_f-Make.md#type-elt) and [`attrib`](./Svg_f-Make.md#type-attrib) are know as *phantom types*. The implementation, defined here, is actually monomorphic.

In particular, tyxml doesn't impose any overhead over the underlying representation. The [`tot`](./Svg_f-Make.md#val-tot) and [`toelt`](./Svg_f-Make.md#val-toelt) functions allows to convert between the typed and the untyped representation without any cost.
