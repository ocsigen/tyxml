...
The type variables in [`elt`](./Tyxml_html.md#type-elt) and [`attrib`](./Tyxml_html.md#type-attrib) are know as *phantom types*. The implementation, defined here, is actually monomorphic.

In particular, tyxml doesn't impose any overhead over the underlying representation. The [`tot`](./Tyxml_html.md#val-tot) and [`toelt`](./Tyxml_html.md#val-toelt) functions allows to convert between the typed and the untyped representation without any cost.
...
```
```ocaml
type touch_event_handler = Tyxml_xml.touch_event_handler
```
```ocaml
