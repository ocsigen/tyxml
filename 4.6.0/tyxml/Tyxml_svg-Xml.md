...
The type variables in [`elt`](./Tyxml_svg.md#type-elt) and [`attrib`](./Tyxml_svg.md#type-attrib) are know as *phantom types*. The implementation, defined here, is actually monomorphic.

In particular, tyxml doesn't impose any overhead over the underlying representation. The [`tot`](./Tyxml_svg.md#val-tot) and [`toelt`](./Tyxml_svg.md#val-toelt) functions allows to convert between the typed and the untyped representation without any cost.
