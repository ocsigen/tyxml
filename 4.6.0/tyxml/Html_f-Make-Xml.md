...
The type variables in [`elt`](./Html_f-Make.md#type-elt) and [`attrib`](./Html_f-Make.md#type-attrib) are know as *phantom types*. The implementation, defined here, is actually monomorphic.

In particular, tyxml doesn't impose any overhead over the underlying representation. The [`tot`](./Html_f-Make.md#val-tot) and [`toelt`](./Html_f-Make.md#val-toelt) functions allows to convert between the typed and the untyped representation without any cost.
