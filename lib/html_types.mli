...
(** HTML types with variants, goes with {!Html_sigs.T}.
    ...
    @see <http://dev.w3.org/html5/spec/Overview.html>
      information concerning HTML at W3C. *)
...
(** Authors may use the following recognized link types, listed here with their
    ...
    list of link types. White space characters are not permitted within link
    types. These link types are case-insensitive, i.e., ["Alternate"] has the
    ...
    - [`Archives]: Provides a link to a collection of records, documents, or
    ...
    - [`Canonical]: Gives the preferred location for accessing the current
      document.
    - [`External]: Indicates that the referenced document is not part of the
      same site as the current document.
    - [`First]: Indicates that the current document is a part of a series, and
    ...
    - [`Index]: Gives a link to the document that provides a table of contents
      or index listing the current document.
    - [`Last]: Indicates that the current document is a part of a series, and
      that the last document in the series is the referenced document.
    - [`Licence]: Indicates that the main content of the current document is
      covered by the copyright license described by the referenced document.
    - [`Next]: Indicates that the current document is a part of a series, and
      that the next document in the series is the referenced document.
    - [`Nofollow]: Indicates that the current document's original author or
      publisher does not endorse the referenced document.
    - [`Noreferrer]: Requires that the user agent not send an HTTP Referer (sic)
      header if the user follows the hyperlink.
    - [`Noopener]: Instructs the browser to open the link without granting the
      new browsing context access to the document that opened it.
    - [`Pingback]: Gives the address of the pingback server that handles
      pingbacks to the current document.
    - [`Prefetch]: Specifies that the target resource should be preemptively
      cached.
    - [`Prev]: Indicates that the current document is a part of a series, and
      that the previous document in the series is the referenced document.
    - [`Search]: Gives a link to a resource that can be used to search through
      the current document and its related pages.
    - [`Stylesheet]: Imports a stylesheet.
    - [`Sidebar]: Specifies that the referenced document, if retrieved, is
      intended to be shown in the browser's sidebar (if it has one).
    - [`Tag]: Gives a tag (identified by the given address) that applies to the
      current document.
    - [`Up]: Provides a link to a document giving the context for the current
      document. *)
...
(** The MediaDesc attribute is a comma-separated list of media descriptors. The
    ...
    - [`TTY]: For media using a fixed-pitch character grid (like teletypes,
      terminals, or devices with limited display capabilities).
    ...
    - [`Print]: For paged and for documents viewed on screen in print preview
      mode.
...
(** The value is an integer that represents the number of pixels of the canvas
    ...
    information about the definition of a pixel, please consult CSS2.

    @see <http://www.w3.org/TR/1998/REC-CSS2-19980512> CSS2 *)
type pixels = int

(** Script data can be the content of the ["script"] element and the value of
    intrinsic event attributes. User agents must not evaluate script data as
    HTML markup but instead must pass it on as data to a script engine.
...
(** Transparent elements. Such elements have a part of they children in their
    data and behaves like them. We could do something like
    [a: 'a elt list -> 'a elt] but the information about the node name would be
    forgotten and would allow things like that : [p \[a \[a \[\]\]\]]. This
    system allow to build non-conforming terms such as [a \[a \[\]\]] but when
    passed to a standard element (such as [p]), it will yield an error.
...
(** Interactive contents : contents that require user-interaction (Forms, link,
    etc.) *)
...
type audio_attrib = [ common | media_attrib ]
...
type video_attrib = [ common | media_attrib | `Poster | `Width | `Height ]
...
type meta_attrib =
  [ common | `Http_equiv | `Name | `Content | `Charset | `Property ]
