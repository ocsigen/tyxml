...
module Import (Xml : Xml_sigs.T) = struct
  let of_list l =
    List.fold_right (fun a b -> Xml.W.(cons (return a) b)) l (Xml.W.nil ())
  ...
  let rec mk children (seq : signal Seq.t) =
    match seq () with
    | Cons (`Comment s, q) -> mk (Xml.comment s :: children) q
    | Cons (`Text s, q) ->
    ...
    | Cons (`End_element, rest) -> (of_list (List.rev children), rest)
    | Nil -> (of_list (List.rev children), Seq.empty)

  let of_seq seq =
    let l, rest = mk [] seq in
    match rest () with Seq.Nil -> l | _ -> raise Malformed_stream
end
