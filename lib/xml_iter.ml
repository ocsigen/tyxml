...
module Make (Xml : Xml_sigs.Iterable) = struct
  ...
  let rec add_float_attrib name value = function
    ...
    | head :: tail when aname head = name -> float_attrib name value :: tail
    | head :: tail -> head :: add_float_attrib name value tail

  let map_float_attrib is_attrib f l =
    let aux head =
      match acontent head with
      | AFloat value when is_attrib (aname head) ->
          float_attrib (aname head) (f value)
      | _ -> head
    in
    List.map aux l
  ...
  let rec add_int_attrib name value = function
    ...
    | head :: tail when aname head = name -> int_attrib name value :: tail
  ...
  let map_int_attrib is_attrib f l =
    let aux head =
      match acontent head with
      | AInt value when is_attrib (aname head) ->
          int_attrib (aname head) (f value)
      | _ -> head
    in
    List.map aux l
  ...
  let rec add_string_attrib name value = function
    ...
    | head :: tail when aname head = name -> string_attrib name value :: tail
    | head :: tail -> head :: add_string_attrib name value tail

  let map_string_attrib is_attrib f l =
    let aux head =
      match acontent head with
      | AStr value when is_attrib (aname head) ->
          string_attrib (aname head) (f value)
      | _ -> head
    in
    List.map aux l
  ...
  let rec add_space_sep_attrib name value = function
    ...
    | head :: tail -> (
        ...
        | _ when aname head = name -> space_sep_attrib name [ value ] :: tail
  ...
  let rec add_comma_sep_attrib name value = function
    ...
    | head :: tail -> (
        ...
        | _ when aname head = name -> comma_sep_attrib name [ value ] :: tail
  ...
  let rec rm_attrib_from_list is_attrib is_value = function
    ...
    | head :: tail -> (
        ...
        | AStrL (sep, values) when is_attrib (aname head) -> (
            match List.filter (fun v -> not (is_value v)) values with
            | [] -> tail
            | values' -> (
                match sep with
  ...
  let rec fold of_empty of_comment of_txt of_encodedpcdata of_entity of_leaf
      of_node n
    =
    match content n with
    | Empty -> of_empty ()
    ...
    | Node (name, attribs, elts) ->
        of_node name attribs
          (List.map
             (fold of_empty of_comment of_txt of_encodedpcdata of_entity of_leaf
                of_node)
             elts)
  ...
  let all_entities elt =
    ...
    fold f f f f f
      (fun _ename _attribs -> [])
      (fun _ename _attribs elts -> List.flatten elts)
      elt
  ...
  let translate root_leaf root_node sub_leaf sub_node update_state state n =
    let rec translate' state n =
      match content n with
      | Empty | Comment _ | PCDATA _ | EncodedPCDATA _ | Entity _ -> [ n ]
      | Leaf (name, attribs) -> sub_leaf state name attribs
      | Node (name, attribs, elts) ->
    ...
    match content n with
    | Empty | Comment _ | PCDATA _ | EncodedPCDATA _ | Entity _ -> n
    | Leaf (name, attribs) -> root_leaf name attribs
    | Node (name, attribs, elts) ->
