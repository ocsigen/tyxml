...
struct
  ...
  module Info = struct
    ...
    let doctype = Xml_print.compose_doctype "html" []
  ...
  let constant_attrib a () = string_attrib a (W.return a)
  let linktypes_attrib name x = user_attrib C.string_of_linktypes name x
  let mediadesc_attrib name x = user_attrib C.string_of_mediadesc name x
  let srcset_attrib name x = user_attrib C.string_of_srcset name x
  ...
  let a_xmlns x = user_attrib C.string_of_big_variant "xmlns" x
  let a_manifest = uri_attrib "manifest"
  let a_cite = uri_attrib "cite"
  let a_xml_space x = user_attrib C.string_of_big_variant "xml:space" x
  let a_accesskey c = user_attrib C.string_of_character "accesskey" c
  ...
  let a_selected = constant_attrib "selected"
  ...
  let a_method x = user_attrib C.string_of_big_variant "method" x
  let a_formmethod = a_method
  let a_enctype = string_attrib "enctype"
  let a_checked = constant_attrib "checked"
  let a_disabled = constant_attrib "disabled"
  let a_readonly = constant_attrib "readonly"
  let a_maxlength = int_attrib "maxlength"
  let a_minlength = int_attrib "minlength"
  let a_name = string_attrib "name"
  let a_allowfullscreen = constant_attrib "allowfullscreen"
  let a_allowpaymentrequest = constant_attrib "allowpaymentrequest"

  let a_referrerpolicy x =
    user_attrib C.string_of_referrerpolicy "referrerpolicy" x

  let a_autocomplete x = user_attrib C.string_of_autocomplete "autocomplete" x
  let a_async = constant_attrib "async"
  let a_autofocus = constant_attrib "autofocus"
  let a_autoplay = constant_attrib "autoplay"
  let a_muted = constant_attrib "muted"
  ...
  let a_contenteditable ce = bool_attrib "contenteditable" ce
  let a_contextmenu = string_attrib "contextmenu"
  let a_controls = constant_attrib "controls"
  let a_dir x = user_attrib C.string_of_big_variant "dir" x
  let a_draggable d = bool_attrib "draggable" d
  let a_form = string_attrib "form"
  let a_formaction = uri_attrib "formaction"
  let a_formenctype = string_attrib "formenctype"
  let a_formnovalidate = constant_attrib "formnovalidate"
  let a_formtarget = string_attrib "formtarget"
  let a_hidden = constant_attrib "hidden"
  let a_high = float_attrib "high"
  let a_icon = uri_attrib "icon"
  let a_ismap = constant_attrib "ismap"
  let a_keytype = string_attrib "keytype"
  let a_list = string_attrib "list"
  let a_loop = constant_attrib "loop"
  ...
  let a_inputmode x = user_attrib C.string_of_big_variant "inputmode" x
  let a_novalidate = constant_attrib "novalidate"
  let a_open = constant_attrib "open"
  ...
  let a_preload x = user_attrib C.string_of_big_variant "preload" x
  let a_pubdate = constant_attrib "pubdate"
  let a_radiogroup = string_attrib "radiogroup"
  let a_required = constant_attrib "required"
  let a_reversed = constant_attrib "reserved"
  let a_sandbox x = user_attrib C.string_of_sandbox "sandbox" x
  let a_spellcheck sc = bool_attrib "spellcheck" sc
  let a_scoped = constant_attrib "scoped"
  let a_seamless = constant_attrib "seamless"
  let a_sizes sizes = user_attrib C.string_of_sizes "sizes" sizes
  ...
  let a_step step = user_attrib C.string_of_step "step" step
  let a_translate x = user_attrib C.string_of_big_variant "translate" x
  let a_wrap x = user_attrib C.string_of_big_variant "wrap" x
  let a_size = int_attrib "size"
  let a_input_type it = user_attrib C.string_of_input_type "type" it
  let a_menu_type x = user_attrib C.string_of_big_variant "type" x
  let a_command_type x = user_attrib C.string_of_big_variant "type" x
  let a_button_type bt = user_attrib C.string_of_input_type "type" bt
  ...
  let a_align x = user_attrib C.string_of_big_variant "align" x
  ...
  let a_scope x = user_attrib C.string_of_big_variant "scope" x
  let a_border = int_attrib "border"
  let a_rules x = user_attrib C.string_of_big_variant "rules" x
  let a_char c = user_attrib C.string_of_character "char" c
  let a_data = uri_attrib "data"
  let a_codetype = string_attrib "codetype"
  let a_frameborder x = user_attrib C.string_of_big_variant "frameborder" x
  let a_marginheight = int_attrib "marginheight"
  let a_marginwidth = int_attrib "marginwidth"
  let a_scrolling x = user_attrib C.string_of_big_variant "scrolling" x
  ...
  let unary tag ?a elt = Xml.node ?a tag (W.singleton elt)
  let star tag ?a elts = Xml.node ?a tag elts
  let plus tag ?a elt elts = Xml.node ?a tag (W.cons elt elts)
  ...
  let a_shape x = user_attrib C.string_of_big_variant "shape" x
  let a_coords coords = user_attrib C.string_of_numbers "coords" coords
  let a_usemap = string_attrib "usemap"
  let a_defer = constant_attrib "defer"
  ...
  let video_audio name ?src ?srcs ?(a = []) elts =
    let a = match src with None -> a | Some uri -> a_src uri :: a in
    match srcs with
  ...
  let command ~label ?(a = []) () = Xml.leaf ~a:(a_label label :: a) "command"
  ...
  let form = star "form"
  let svg ?(a = []) children = Svg.toelt (Svg.svg ~a children)
  ...
  let datalist ?children ?a () =
    let children =
      match children with None -> W.nil () | Some (`Options x | `Phras x) -> x
    in
    Xml.node ?a "datalist" children

  let progress = star "progress"
  let legend = star "legend"
  let details summary ?a children = plus "details" ?a summary children
  ...
  let tablex ?caption ?columns ?thead ?tfoot ?a elts =
    ...
    let content =
      match columns with
      | None -> content
      | Some columns -> W.append columns content
    in
  ...
  let object_ ?params ?(a = []) elts =
    let elts = match params with None -> elts | Some e -> W.append e elts in
...
struct
  ...
  let onoff_of_bool = function false -> "off" | true -> "on"
  ...
  let string_of_linktypes l = String.concat " " (List.map string_of_linktype l)
  ...
  let string_of_srcset (l : [< image_candidate ] list) =
    let f = function
      ...
      | `Url_pixel (url, v) ->
          Printf.sprintf "%s %sx" (Xml.string_of_uri url)
            (Xml_print.string_of_number v)
    in
