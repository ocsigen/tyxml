...
(** This module defines basic data types for data, attributes and element
    occurring in SVG documents. It is based on the specification available at
...
module Unit = struct
  ...
  let to_string f (n, unit) =
    Printf.sprintf "%g%s" n (match unit with Some unit -> f unit | None -> "")
...
let opt_concat ?(sep = " ") s f = function Some x -> s ^ sep ^ f x | None -> s
...
struct
  ...
  let toeltl x = x
  let to_attrib x = x
  let nullary tag ?a () = Xml.node ?a tag (W.nil ())
  ...
  let a_preserveAspectRatio = string_attrib "preserveAspectRatio"
  let a_contentScriptType = string_attrib "contentScriptType"
  let a_contentStyleType = string_attrib "contentStyleType"
  let a_zoomAndPan x = user_attrib C.string_of_big_variant "zoomAndSpan" x
  let a_href = string_attrib "href"
  let a_xlink_href = string_attrib "xlink:href"
  let a_requiredFeatures = Xml.space_sep_attrib "requiredFeatures"
  let a_requiredExtensions = Xml.space_sep_attrib "requiredExtension"
  let a_systemLanguage = Xml.comma_sep_attrib "systemLanguage"
  ...
  let a_lengthAdjust x = user_attrib C.string_of_big_variant "lengthAdjust" x
  let a_textLength = user_attrib string_of_length "textLength"
  let a_text_anchor x = user_attrib C.string_of_big_variant "text-anchor" x
  ...
  let a_spacing x = user_attrib C.string_of_big_variant "spacing" x
  let a_glyphRef = string_attrib "glyphRef"
  let a_format = string_attrib "format"
  let a_markerUnits x = user_attrib C.string_of_big_variant "markerUnits" x
  ...
  let a_orient x = user_attrib C.string_of_orient "orient" x
  let a_local = string_attrib "local"

  let a_rendering_intent x =
    user_attrib C.string_of_big_variant "rendering-intent" x

  let a_gradientUnits x = user_attrib C.string_of_big_variant "gradientUnits" x

  let a_gradientTransform =
    user_attrib C.string_of_transforms "gradientTransform"

  let a_spreadMethod x = user_attrib C.string_of_big_variant "spreadMethod" x
  let a_fx = user_attrib string_of_coord "fx"
  let a_fy = user_attrib string_of_coord "fy"
  let a_offset x = user_attrib C.string_of_offset "offset" x
  let a_patternUnits x = user_attrib C.string_of_big_variant "patternUnits" x
  ...
  let a_clipPathUnits x = user_attrib C.string_of_big_variant "clipPathUnits" x
  let a_maskUnits x = user_attrib C.string_of_big_variant "maskUnits" x
  ...
  let a_in x = user_attrib C.string_of_in_value "in" x
  let a_in2 x = user_attrib C.string_of_in_value "in2" x
  ...
  let a_mode x = user_attrib C.string_of_big_variant "mode" x
  let a_feColorMatrix_type x = user_attrib C.string_of_big_variant "type" x
  let a_values = user_attrib C.string_of_numbers "values"
  let a_transfer_type x = user_attrib C.string_of_big_variant "type" x
  ...
  let a_edgeMode x = user_attrib C.string_of_big_variant "targetY" x
  ...
  let a_diffuseConstant = user_attrib C.string_of_number "diffuseConstant"
  ...
  let a_stitchTiles x = user_attrib C.string_of_big_variant "stitchTiles" x
  let a_feTurbulence_type x = user_attrib C.string_of_big_variant "type" x
  let a_xlink_show x = user_attrib C.string_of_big_variant "xlink:show" x
  let a_xlink_actuate x = user_attrib C.string_of_big_variant "xlink:actuate" x
  ...
  let a_attributeType x = user_attrib C.string_of_big_variant "attributeType" x
  let a_begin = string_attrib "begin"
  let a_dur = string_attrib "dur"
  let a_min = string_attrib "min"
  let a_max = string_attrib "max"
  let a_restart x = user_attrib C.string_of_big_variant "restart" x
  ...
  let a_animation_fill x = user_attrib C.string_of_big_variant "fill" x
  let a_fill_rule = user_attrib C.string_of_fill_rule "fill-rule"
  let a_calcMode x = user_attrib C.string_of_big_variant "calcMode" x
  ...
  let a_additive x = user_attrib C.string_of_big_variant "additive" x
  let a_accumulate x = user_attrib C.string_of_big_variant "accumulate" x
  let a_keyPoints = user_attrib C.string_of_numbers_semicolon "keyPoints"
  let a_path = string_attrib "path"
  let a_animateTransform_type = user_attrib C.string_of_big_variant "type"
  ...
  let a_orientation x = user_attrib C.string_of_big_variant "orientation" x
  let a_arabic_form x = user_attrib C.string_of_big_variant "arabic-form" x
  ...
  let a_underline_position = user_attrib C.string_of_number "underline-position"
  ...
  let a_overline_thickness = user_attrib C.string_of_number "overline-thickness"
  ...
  let a_stroke_miterlimit = user_attrib C.string_of_number "stroke-miterlimit"

  let a_stroke_dasharray x =
    user_attrib C.string_of_strokedasharray "stroke-dasharray" x

  let a_stroke_dashoffset = user_attrib C.string_of_length "stroke-dashoffset"
  let a_stroke_opacity = user_attrib C.string_of_number "stroke-opacity"
