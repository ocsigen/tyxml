...
(** Printing utilities.
    ...
    This module contains various encoding functions that can be used with
    {!Tyxml.Html.pp} and {!Tyxml.Svg.pp}.

    It also contains functors to create printers for your own XML data
    structure. *)
...
(** The encoder maps strings to HTML and {e must} encode the unsafe characters
    ...
    to HTML entities. [encode_unsafe_char] is the default for [?encode] in
    [output] and [pretty_print] below. Other implementations are provided by the
    module [Netencoding] in the
    {{:http://www.ocaml-programming.de/programming/ocamlnet.html} OcamlNet}
    library, e.g.:

    {[
      let encode =
        Netencoding.Html.encode ~in_enc:`Enc_iso88591 ~out_enc:`Enc_usascii ()
    ]}

    Where national characters are replaced by HTML entities. The user is of
    course free to write her own implementation.
...
(** Utf8 normalizer and encoder for HTML.
    ...
    Given a [pp] function produced by one of the functors in {!Xml_print}, this
    modules is used as following:
...
module Utf8 : sig
  ...
  (** [normalize str] take a possibly invalid utf-8 string and return a valid
      utf-8 string where invalid bytes have been replaced by the replacement
      character [U+FFFD]. The returned boolean is true if invalid bytes were
      found *)

  val normalize_html : string -> utf8 * bool
  (** Same as [normalize] plus some extra work : It encode '<' , '>' , '"' , '&'
      characters with corresponding entities and replaced invalid html character
      by [U+FFFD] *)
end
...
(** [encoding] is the name of the character encoding, e.g. ["US-ASCII"] or
    ["UTF-8"] *)
...
(** Printers for typed XML modules such as the one produced by {!Svg_f} and
    {!Html_f}. *)
...
module Make_fmt (Xml : Xml_sigs.Iterable) (I : TagList) :
...
module Make_simple (Xml : Xml_sigs.Iterable) (I : TagList) :
