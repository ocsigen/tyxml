...
let compose_decl ?(version = "1.0") ?(encoding = "UTF-8") () =
  Format.sprintf {|<?xml version="%s" encoding="%s"?>\n|} version encoding
  ...
  let pp_args fmt = function
    ...
    | l ->
        Format.fprintf fmt " PUBLIC %a"
          ...
          l
  in
  Format.asprintf "<!DOCTYPE %s%a>" dt pp_args args

let re_end_comment =
  Re.(
    compile
    @@ alt
         [ seq [ bos; str ">" ]; seq [ bos; str "->" ]; str "-->"; str "--!>" ])
...
let pp_number fmt v =
  if v = infinity then Format.pp_print_string fmt "Infinity"
  else if v = neg_infinity then Format.pp_print_string fmt "-Infinity"
  else if v <> v then Format.pp_print_string fmt "NaN"
  else
    let vint = int_of_float v in
    (* compiler 1000 into 1e3 *)
    if float_of_int vint = v then
      let rec div n i =
        if n <> 0 && n mod 10 = 0 then div (n / 10) (succ i)
        else if i > 2 then Format.fprintf fmt "%de%d" n i
        else Format.pp_print_int fmt vint
      in
      div vint 0
    else
      let s1 = Printf.sprintf "%.12g" v in
      if v = float_of_string s1 then Format.pp_print_string fmt s1
      else
        let s2 = Printf.sprintf "%.15g" v in
        if v = float_of_string s2 then Format.pp_print_string fmt s2
        else Format.fprintf fmt "%.18g" v

let string_of_number v = Format.asprintf "%a" pp_number v
...
module Utf8 = struct
  ...
  let normalization_needed src =
    let rec loop src i l =
      ...
      | '\034' | '\038' | '\060' | '\062' -> true
      (* ASCII characters *)
      | '\009' | '\010' | '\013' | '\032' .. '\126' -> loop src (i + 1) l
      | _ -> true
    in
    loop src 0 (String.length src)

  let normalize_html src =
    if normalization_needed src then (
      let warn = ref false in
      let buffer = Buffer.create (String.length src) in
      Uutf.String.fold_utf_8
        (fun _ _ d ->
...
module Make_fmt (Xml : Xml_sigs.Iterable) (I : TagList) = struct
  open Xml
  ...
  let sp indent fmt =
    if indent then Format.pp_print_space fmt ()
    else Format.pp_print_string fmt " "

  let cut indent fmt = if indent then Format.pp_print_cut fmt () else ()

  module S = Set.Make (String)

  let is_emptytag =
    match I.emptytags with
    | [] -> fun _ -> false
    | l ->
  ...
  let pp_encode encode indent fmt s =
    let s = encode s in
    if indent then Format.fprintf fmt "@[%a@]" Format.pp_print_text s
  ...
  let pp_attrib encode indent fmt a =
    Format.fprintf fmt "%t%s=%a" (sp indent) (aname a)
  ...
  let pp_tag_and_attribs encode indent fmt (tag, attrs) =
    open_box indent fmt;
    Format.fprintf fmt "%s%a%t" tag (pp_attribs encode indent) attrs
      (cut indent);
  ...
  let rec pp_tag encode indent fmt tag attrs children =
    ...
    | _ ->
        open_box indent fmt;
        Format.fprintf fmt "<%t%a>%t%a%t%t</%s>" (open_box indent)
          (pp_tag_and_attribs encode indent)
          (tag, attrs) (cut indent) (pp_elts encode indent) children
          (close_box indent) (cut indent) tag;
        close_box indent fmt

  and pp_elt encode indent fmt elt =
    match content elt with
    | Comment texte -> Format.fprintf fmt "<!--%s-->" (escape_comment texte)
    | Entity e -> Format.fprintf fmt "&%s;" e
    | PCDATA texte -> pp_encode encode indent fmt texte
    | EncodedPCDATA texte -> Format.pp_print_string fmt texte
    | Node (name, xh_attrs, xh_taglist) ->
        pp_tag encode indent fmt name xh_attrs xh_taglist
    | Leaf (name, xh_attrs) -> pp_closedtag encode indent fmt name xh_attrs
    | Empty -> ()
...
struct
  ...
  let prepare_document doc =
    ...
    | Xml.Node (n, a, c) ->
        let a =
          if List.exists (fun a -> Xml.aname a = "xmlns") a then a
...
module Make (Xml : Xml_sigs.Iterable) (I : TagList) (O : Xml_sigs.Output) =
struct
  let ( ++ ) = O.concat
  ...
  let attrib_value_to_string encode a =
    match acontent a with
  ...
  let rec xh_print_attrs encode attrs =
    match attrs with
    | [] -> O.empty
    | attr :: queue ->
  ...
  and xh_print_closedtag encode tag attrs =
    if I.emptytags = [] || List.mem tag I.emptytags then
      O.put ("<" ^ tag) ++ xh_print_attrs encode attrs ++ O.put " />"
    else
      O.put ("<" ^ tag)
  ...
  and xh_print_tag encode tag attrs taglist =
    if taglist = [] then xh_print_closedtag encode tag attrs
    else
      O.put ("<" ^ tag)
      ++ xh_print_attrs encode attrs
      ++ O.put ">"
  ...
  and xh_print_taglist encode taglist =
    ...
    | elt :: queue -> (
        ...
        | Entity e ->
            O.put ("&" ^ e ^ ";") (* no encoding *)
            ++ xh_print_taglist encode queue
        | PCDATA texte -> O.put (encode texte) ++ xh_print_taglist encode queue
        | EncodedPCDATA texte -> O.put texte ++ xh_print_taglist encode queue
        | Node (name, xh_attrs, xh_taglist) ->
            print_nodes encode name xh_attrs xh_taglist queue
        | Leaf (name, xh_attrs) -> print_nodes encode name xh_attrs [] queue
        | Empty -> xh_print_taglist encode queue)
...
module Make_simple (Xml : Xml_sigs.Iterable) (I : TagList) = struct
  let print_list ~output =
    let module M =
      Make (Xml) (I)
        (Simple_output (struct
          let put = output
        end))
    in
    M.print_list
