...
module type T = sig
  ...
  (** HTML elements.
      ...
      Element constructors are in section {{!elements} elements}. Most elements
      constructors are either {{!nullary} nullary}, {{!unary} unary} or {{!star}
      star}, depending on the number of children they accept. Children are
      usually given as a list of elements. {{!txt} txt} is used for text.

      [div \[a \[txt "Foo"\]\]] is equivalent to [<div><a>foo</a></div>]

      The type variable ['a] is used to track the element's type. This allows
      the OCaml typechecker to check HTML validity.

      For example, [div \[\]] is of type [\[> `Div\] elt]. The {{!span} span}
      function only accepts children of type {!Html_types.span_content}. Since
      [`Div] is not part of it. [span \[div \[\]\]] will not typecheck.
  ...
  (** HTML attributes
      ...
      Attribute constructors are in section {{!attributes} attributes} and their
      name starts with [a_]. Attributes are given to elements with the [~a]
      optional argument.

      [a ~a:\[a_href "ocsigen.org"\] \[txt "link!"\]] is equivalent to
      [<a href="ocsigen.org">link!</a>]

      Similarly to {{!elt} elt}, attributes use the OCaml type system to enforce
      HTML validity.

      For example {!a_href} returns a value of type [\[> `Href\] attrib]. The
      {{!div} div} function only accepts attributes of type
      {!Html_types.div_attrib}. Since [`Href] is not part of it,
      [div ~a:\[a_href "ocsigen.org"\] \[\]] will not typecheck.

      In some cases, attributes have to be disambiguated. The [max] attribute
      has two version, {!a_max} and {!a_input_max}, depending on the element.
      Such disambiguated attribute will contain the name of the associated
      element. *)
  type +'a attrib
  ...
  (** Underlying XML data-structure
      ...
      Note that some implementation may not be iterable or printable, such as
      the Dom representation exposed by js_of_ocaml. *)
  ...
  (** [list_wrap] is a container for list of elements.
      ...
      In most cases, ['a list_wrap = 'a list]. For [R] modules (in eliom or
      js_of_ocaml), It will be {!ReactiveData.RList.t}. *)
  ...
  (** A star element is an element that has any number of children, including
      zero. *)
  ...
  (** May be used to specify custom attributes. The example given by the W3C is
      as follows :
...
<ol>
  ...
  (** This attribute offers advisory information about the element for which it
      ...
      {i tool tip} (a short message that appears when the pointing device pauses
      ...
      element to designate an external style sheet. Please consult the section
      on links and style sheets for details. *)
  ...
  (** @see <https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/translate>
        [translate] global attribute documentation. *)
  ...
  (** This attribute is used to describe a reverse link from the anchor
      specified by the href attribute to the current document. The value of this
  ...
  (** This attribute gives an advisory hint as to the content type of the
      ...
      support.Authors who use this attribute take responsibility to manage the
  ...
  val head
    ...
    [< title ] elt wrap ->
    head_content_fun elt list_wrap ->
    [> head ] elt
  ...
  val blockquote
    : ( [< blockquote_attrib ],
        [< blockquote_content_fun ],
        [> blockquote ] )
      star

  val dialog : ([< dialog_attrib ], [< dialog_content_fun ], [> dialog ]) star
  ...
  val picture
    :  img:[< img ] elt wrap ->
    ([< picture_attrib ], [< picture_content_fun ], [> picture ]) star
  ...
  (** Label authorizes only one control inside them that should be labelled with
      a [for] attribute (although it is not necessary). Such constraints are not
      currently enforced by the type-system *)
  ...
  val datalist
    ...
    [@@reflect.filter_whitespace] [@@reflect.element "datalist"]
  ...
  val meter : ([< meter_attrib ], [< meter_content_fun ], [> meter ]) star
  ...
  (** [entity "foo"] is the HTML entity [&foo;]. Both numerical and named form
      are allowed.
  ...
  val command : label:text wrap -> ([< command_attrib ], [> command ]) nullary
  ...
  val script : ([< script_attrib ], [< script_content_fun ], [> script ]) unary
  ...
  (** [import signal] converts the given XML signal into Tyxml elements. It can
      be used with HTML and SVG parsing libraries, such as Markup.
  ...
  (** Unsafe features.
      ...
      Using this module can break HTML validity and may introduce security
      problems like code injection. Use it with care. *)
  module Unsafe : sig
    val data : string wrap -> 'a elt
    (** Insert raw text without any encoding *)

    val node : string -> ?a:'a attrib list -> 'b elt list_wrap -> 'c elt
    (** Insert an XML node that is not implemented in this module. If it is a
        standard HTML node which is missing, please report to the Ocsigen team. *)

    val leaf : string -> ?a:'a attrib list -> unit -> 'b elt
    (** Insert an XML node without children that is not implemented in this
        module. If it is a standard HTML node which is missing, please report to
        the Ocsigen team. *)

    val coerce_elt : 'a elt -> 'b elt
    (** Remove phantom type annotation on an element, to make it usable
        everywhere. *)

    val string_attrib : string -> string wrap -> 'a attrib
    (** Insert an attribute that is not implemented in this module. If it is a
        standard HTML attribute which is missing, please report to the Ocsigen
        team. *)
...
module Make (Xml : Xml_sigs.T) (Svg : Svg_sigs.T with module Xml := Xml) : sig
