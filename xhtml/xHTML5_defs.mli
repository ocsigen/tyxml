...
(** Signatures of typesafe constructors for HTML5 documents.
...
module type T = sig
  ...
  (** Abstraction over XML and SVG's types *)
  type raw_xml_elt
  type raw_xml_attrib
  type raw_xml_event
  type 'a raw_svg_elt
  type 'a raw_svg_attrib
...
(** {1 Common Attributes} *)
  ...
  val to_xmlattribs : ('a attrib) list -> raw_xml_attrib list
...
(** {2 Events} *)
  ...
  (** Javascript events *)
  val a_onabort : raw_xml_event -> [> | `OnAbort] attrib

  val a_onafterprint : raw_xml_event -> [> | `OnAfterPrint] attrib

  val a_onbeforeprint : raw_xml_event -> [> | `OnBeforePrint] attrib

  val a_onbeforeunload : raw_xml_event -> [> | `OnBeforeUnload] attrib

  val a_onblur : raw_xml_event -> [> | `OnBlur] attrib

  val a_oncanplay : raw_xml_event -> [> | `OnCanPlay] attrib

  val a_oncanplaythrough : raw_xml_event -> [> | `OnCanPlayThrough] attrib

  val a_onchange : raw_xml_event -> [> | `OnChange] attrib

  val a_onclick : raw_xml_event -> [> | `OnClick] attrib

  val a_oncontextmenu : raw_xml_event -> [> | `OnContextMenu] attrib

  val a_ondblclick : raw_xml_event -> [> | `OnDblClick] attrib

  val a_ondrag : raw_xml_event -> [> | `OnDrag] attrib

  val a_ondragend : raw_xml_event -> [> | `OnDragEnd] attrib

  val a_ondragenter : raw_xml_event -> [> | `OnDragEnter] attrib

  val a_ondragleave : raw_xml_event -> [> | `OnDragLeave] attrib

  val a_ondragover : raw_xml_event -> [> | `OnDragOver] attrib

  val a_ondragstart : raw_xml_event -> [> | `OnDragStart] attrib

  val a_ondrop : raw_xml_event -> [> | `OnDrop] attrib

  val a_ondurationchange : raw_xml_event -> [> | `OnDurationChange] attrib

  val a_onemptied : raw_xml_event -> [> | `OnEmptied] attrib

  val a_onended : raw_xml_event -> [> | `OnEnded] attrib

  val a_onerror : raw_xml_event -> [> | `OnError] attrib

  val a_onfocus : raw_xml_event -> [> | `OnFocus] attrib

  val a_onformchange : raw_xml_event -> [> | `OnFormChange] attrib

  val a_onforminput : raw_xml_event -> [> | `OnFormInput] attrib

  val a_onhashchange : raw_xml_event -> [> | `OnHashChange] attrib

  val a_oninput : raw_xml_event -> [> | `OnInput] attrib

  val a_oninvalid : raw_xml_event -> [> | `OnInvalid] attrib

  val a_onmousedown : raw_xml_event -> [> | `OnMouseDown] attrib

  val a_onmouseup : raw_xml_event -> [> | `OnMouseUp] attrib

  val a_onmouseover : raw_xml_event -> [> | `OnMouseOver] attrib

  val a_onmousemove : raw_xml_event -> [> | `OnMouseMove] attrib

  val a_onmouseout : raw_xml_event -> [> | `OnMouseOut] attrib

  val a_onmousewheel : raw_xml_event -> [> | `OnMouseWheel] attrib

  val a_onoffline : raw_xml_event -> [> | `OnOffLine] attrib

  val a_ononline : raw_xml_event -> [> | `OnOnLine] attrib

  val a_onpause : raw_xml_event -> [> | `OnPause] attrib

  val a_onplay : raw_xml_event -> [> | `OnPlay] attrib

  val a_onplaying : raw_xml_event -> [> | `OnPlaying] attrib

  val a_onpagehide : raw_xml_event -> [> | `OnPageHide] attrib

  val a_onpageshow : raw_xml_event -> [> | `OnPageShow] attrib

  val a_onpopstate : raw_xml_event -> [> | `OnPopState] attrib

  val a_onprogress : raw_xml_event -> [> | `OnProgress] attrib

  val a_onratechange : raw_xml_event -> [> | `OnRateChange] attrib

  val a_onreadystatechange : raw_xml_event -> [> | `OnReadyStateChange] attrib

  val a_onredo : raw_xml_event -> [> | `OnRedo] attrib

  val a_onresize : raw_xml_event -> [> | `OnResize] attrib

  val a_onscroll : raw_xml_event -> [> | `OnScroll] attrib

  val a_onseeked : raw_xml_event -> [> | `OnSeeked] attrib

  val a_onseeking : raw_xml_event -> [> | `OnSeeking] attrib

  val a_onselect : raw_xml_event -> [> | `OnSelect] attrib

  val a_onshow : raw_xml_event -> [> | `OnShow] attrib

  val a_onstalled : raw_xml_event -> [> | `OnStalled] attrib

  val a_onstorage : raw_xml_event -> [> | `OnStorage] attrib

  val a_onsubmit : raw_xml_event -> [> | `OnSubmit] attrib

  val a_onsuspend : raw_xml_event -> [> | `OnSuspend] attrib

  val a_ontimeupdate : raw_xml_event -> [> | `OnTimeUpdate] attrib

  val a_onundo : raw_xml_event -> [> | `OnUndo] attrib

  val a_onunload : raw_xml_event -> [> | `OnUnload] attrib

  val a_onvolumechange : raw_xml_event -> [> | `OnVolumeChange] attrib

  val a_onwaiting : raw_xml_event -> [> | `OnWaiting] attrib

  val a_onkeypress : raw_xml_event -> [> | `OnKeyPress] attrib

  val a_onkeydown : raw_xml_event -> [> | `OnKeyDown] attrib

  val a_onkeyup : raw_xml_event -> [> | `OnKeyUp] attrib

  val a_onload : raw_xml_event -> [> | `OnLoad] attrib

  val a_onloadeddata : raw_xml_event -> [> | `OnLoadedData] attrib

  val a_onloadedmetadata : raw_xml_event -> [> | `OnLoadedMetaData] attrib

  val a_onloadstart : raw_xml_event -> [> | `OnLoadStart] attrib

  val a_onmessage : raw_xml_event -> [> | `OnMessage] attrib
...
(** {1 Combined Element Sets:} *)
  ...
  val svg : ?xmlns : string -> ?a : [< svg_attrib ] raw_svg_attrib list -> [< svg_content ] raw_svg_elt list -> [> svg ] elt
...
(** {2 Link} *)
  ...
  val link :
    ...
    | `Mime_type
	    ...
	    (** [?encode] maps strings to HTML and {e must} encode the
		unsafe characters ['<'], ['>'], ['"'], ['&'] and the
		control characters 0-8, 11-12, 14-31, 127 to HTML
		entities.  [XML.encode_unsafe] is the default for
		[?encode] in [output] and [pretty_print] below.  Other
		implementations are provided by the module
		[Netencoding] in the
		{{:http://www.ocaml-programming.de/programming/ocamlnet.html}OcamlNet}
		library, e.g.: [let encode = Netencoding.Html.encode
		~in_enc:`Enc_iso88591 ~out_enc:`Enc_usascii ()], Where
		national characters are replaced by HTML entities.
		The user is of course free to write her own
		implementation.  @see
		<http://www.ocaml-programming.de/programming/ocamlnet.html>
		OcamlNet *)

	    (** [~encoding] is the official name of the external
			  character set encoding that is used by [outs
			  : string -> unit]. *)
...
(** A hyperlink to the W3C validator, including the logo.
    @see <http://validator.w3.org> Validator *)
		 *)
  val tot : raw_xml_elt -> 'a elt

  val totl : raw_xml_elt list -> ('a elt) list

  val toelt : 'a elt -> raw_xml_elt

  val toeltl : ('a elt) list -> raw_xml_elt list
