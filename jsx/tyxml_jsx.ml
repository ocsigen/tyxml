...
let is_jsx e =
  let f = function { attr_name = { txt = "JSX" } } -> true | _ -> false in
...
let rec filter_map f = function
  | [] -> []
  | a :: q -> (
      match f a with None -> filter_map f q | Some a -> a :: filter_map f q)
...
let extract_element_list transform_expr elements =
  let rec map acc e =
    ...
    | [%expr [%e? child] :: [%e? rest]] ->
        ...
        map (child :: acc) rest
    | e -> List.rev (Common.antiquot (element_mapper transform_expr e) :: acc)
  in
  map [] elements

let extract_children transform_expr args =
  match
    List.find (function Labelled "children", _ -> true | _ -> false) args
  with
...
type attr = { a_name : Common.name; a_value : string value; a_loc : Location.t }
...
let classify_name ~loc hint_lang lid =
  ...
  let parent_lang, elt =
    match
      ( Element.find_assembler (Html, name),
...
let traverse =
  object (self)
    ...
    method! structure_item hint_lang stri =
    ...
    method! expression hint_lang e =
      if (not (is_jsx e)) || not enabled then super#expression hint_lang e
        ...
        | {
         pexp_desc =
           Pexp_apply (({ pexp_desc = Pexp_ident { txt }; _ } as f_expr), args);
        }
          when is_homemade_component txt ->
            let lang =
              match hint_lang with Some l -> l | None -> Common.Html
            in
            let attributes = filter_map (extract_attr ~lang) args in
            let children = extract_children (self#expression hint_lang) args in
            let e = mk_component ~loc ~lang f_expr attributes children in
            e
        (* matches <div foo={bar}> child1 child2 </div>; *)
        | {
         pexp_desc = Pexp_apply ({ pexp_desc = Pexp_ident { txt }; _ }, args);
        } ->
            ...
            in
            let e =
              Element.parse ~loc ~parent_lang ~name ~attributes children
            in
            e
...
let () =
  Ppxlib.Driver.register_transformation ~impl:(traverse#structure None)
    "tyxml-jsx"
