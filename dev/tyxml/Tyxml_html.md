...
see [http://www.w3.org/TR/html5/](http://www.w3.org/TR/html5/) W3C Recommendation
Concrete implementation of Html typesafe constructors. See [`Html_sigs.T`](./Html_sigs-module-type-T.md).
...
Note that the concrete implementation of this type can vary. See [`Xml`](./Tyxml_html-Xml.md) for details.
...
In the DOM, pointer events inherit from mouse events, so the handlers use [`Xml.mouse_event_handler`](./Tyxml_html-Xml.md#type-mouse_event_handler).
...
val a_as : 
  ...
  [> `As ] attrib
```
Destination of a preload link. This covers the destinations listed by MDN; other spec destinations can be produced with [`Unsafe.string_attrib`](./Tyxml_html-Unsafe.md#val-string_attrib).
...
val svg : 
  ...
  [< Html_types.svg_content ] Tyxml_svg.elt list_wrap ->
  [> Html_types.svg ] elt
```
...
```
```ocaml
val _pp_elt : Stdlib.Format.formatter -> _ elt -> unit
```