...
The type variables in [`elt`](./Svg_sigs-Make-module-type-T.md#type-elt) and [`attrib`](./Svg_sigs-Make-module-type-T.md#type-attrib) are know as *phantom types*. The implementation, defined here, is actually monomorphic.

In particular, tyxml doesn't impose any overhead over the underlying representation. The [`tot`](./Svg_sigs-Make-module-type-T.md#val-tot) and [`toelt`](./Svg_sigs-Make-module-type-T.md#val-toelt) functions allows to convert between the typed and the untyped representation without any cost.
