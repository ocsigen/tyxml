...
module FunTyp = struct
  ...
  let contains_elt t =
    let iterate =
      object
        ...
        method! core_type =
          function
          | [%type: [%t? _] elt] -> raise Found | ty -> super#core_type ty
      end
    in
  ...
  let rec to_attribute_parser lang name ~loc = function
    ...
    | [ ([%type: bool] as ty) ]
      when List.exists (fun ty -> ty.attr_name.txt = "onoff") ty.ptyp_attributes
      ->
        [%expr onoff]
    | [ [%type: bool] ] -> [%expr bool]
    | [ [%type: unit] ] -> [%expr nowrap unit]
    | [ [%type: number] ] when lang = `Html -> [%expr int]
    | [ [%type: pixels] ] | [ [%type: int] ] -> [%expr int]
    | [ [%type: numbers] ] when lang = `Html -> [%expr commas int]
    | [ [%type: number] ] when lang = `Svg -> [%expr float]
    | [ [%type: float_number] ] | [ [%type: float] ] -> [%expr float]
    | [ [%type: float_number option] ] -> [%expr option "any" float]
    | [ [%type: numbers_semicolon] ] -> [%expr semicolons float]
    | [ [%type: numbers] ] when lang = `Svg -> [%expr spaces_or_commas float]
    | [ [%type: fourfloats] ] -> [%expr fourfloats]
    | [ [%type: number_optional_number] ] -> [%expr number_pair]
    | [ [%type: coords] ] -> [%expr points]
    | [ [%type: (number * number) list option] ] ->
        [%expr option "any" (spaces icon_size)]
    | [ [%type: coord] ] | [ [%type: Unit.length] ] -> [%expr svg_length]
    | [ [%type: Unit.length list] ] -> [%expr spaces_or_commas svg_length]
    | [ [%type: Unit.angle option] ] -> [%expr option "auto" angle]
    | [ [%type: string] ]
    | [ [%type: text] ]
    | [ [%type: nmtoken] ]
    | [ [%type: idref] ]
    ...
    | [ [%type: referrerpolicy] ] -> [%expr variant_or_empty "Empty"]
    | [ [%type: mediadesc] ] ->
        [%expr commas (total_variant Html_types_reflected.mediadesc_token)]
    | [ [%type: lengths] ] -> [%expr spaces_or_commas svg_length]
    | [ [%type: transforms] ] -> [%expr spaces_or_commas transform]
    | [ [%type: paint] ] -> [%expr paint]
    | [ [%type: number_or_datetime] ] -> [%expr number_or_datetime]
    | [ [%type: image_candidate list] ] -> [%expr commas srcset_element]
    | [ [%type: autocomplete_option] ] -> [%expr autocomplete]
    | _ ->
...
let ocaml_attributes_to_renamed_attribute name attributes =
  ...
  | Some { attr_loc = loc; attr_payload = payload } -> (
      ...
      in
      match payload with
      | PStr [%str [%e? const] [%e? element_names]] -> (
          match get_str const with
          | None -> error ()
          | Some real_name ->
              let element_names =
                let error loc =
...
let val_item_to_element_info lang value_description =
  ...
  let maybe_assembler, real_name =
    match maybe_attribute with
    | Some { attr_loc = loc; attr_payload = payload } -> (
        let assembler, real_name =
          match payload with
          | PStr [%str [%e? assembler] [%e? name]] ->
              (get_str assembler, get_str name)
          | PStr [%str [%e? assembler]] -> (get_str assembler, None)
          | _ -> (None, None)
        in
  ...
  | Some assembler ->
      ...
      let labeled_attributes =
        let aux x acc =
          match FunTyp.extract_attribute_argument x with
          | None -> acc
          | Some (label, ty) ->
              let parser =
                FunTyp.to_attribute_parser lang label [ ty ] ~loc:ty.ptyp_loc
              in
      ...
      let assembler =
        match
          find_attr "reflect.filter_whitespace"
...
class reflector lang =
  object
      ...
      | Psig_value
        ...
        when is_attribute name ->
          ...
          let attribute_parser_mapping =
            (name, FunTyp.to_attribute_parser lang name argument_types ~loc)
          in
          attribute_parsers := attribute_parser_mapping :: !attribute_parsers;

          let renaming =
            ocaml_attributes_to_renamed_attribute name pval_attributes
          in
          renamed_attributes := renaming @ !renamed_attributes
      | Psig_value v -> (
          (* Non-attribute, but potentially an element declaration. *)
          match val_item_to_element_info lang v with
          | None -> ()
          | Some (assembler, labeled_attributes', rename) ->
              element_assemblers :=
                (v.pval_name.txt, assembler) :: !element_assemblers;
    ...
    method! type_declaration declaration =
      ...
      (if List.exists is_reflect declaration.ptype_attributes then
             ...
             |> List.map (function
                  ...
                  | { prf_desc = Rinherit { ptyp_loc } } ->
                      Location.raise_errorf ~loc:ptyp_loc
                        "Inclusion is not supported by \
                         [@@reflect.total_variant]")
           in

           let nullary, unary =
             List.partition (fun (_, types) -> types = []) rows
           in
...
module Combi = struct
  ...
  let rec compose_ids = function
    | [ i ] -> id i
    | i :: tl -> Builder.eapply (id i) [ compose_ids tl ]
    | [] -> assert false
end

(** Create a module based on the various things collected while reading the
    file. *)
let emit_module () =
  let loc = Location.none in
  (if !attribute_parsers <> [] then
   [%str
...
let () =
  ...
  let lang =
    ...
    if
      String.length basename >= String.length svg_prefix
  ...
  try read_sig in_file |> reflected_struct |> write_struct out_file
  with exn ->
