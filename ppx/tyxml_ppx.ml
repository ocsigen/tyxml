    ...
    fun given_markup_location ->
      ...
      let rec bounded_maximum best = function
        ...
        | ((noted_markup_location, _) as loc) :: rest ->
            if
              Markup.compare_locations noted_markup_location
                given_markup_location
              > 0
            then best
...
(** Antiquotations
    ...
    We replace antiquotations expressions by a dummy token "(tyxmlX)". We store
    a table token to expression to retrieve them after parsing. *)
module Antiquot = struct
  ...
  let assert_no_antiquot ~loc kind (_namespace, s) =
    ...
    | `No -> ()
    | `Yes e | `Whole e ->
        Common.error e.pexp_loc "OCaml expressions are not accepted as %s names"
          kind
end
...
let replace_attribute ~loc ((ns, attr_name), value) =
  ...
  | `Yes _ ->
      Common.error loc
        "Mixing literals and OCaml expressions is not supported in attribute \
         values"
...
(** Takes the ast and transforms it into a Markup.ml char stream.
    ...
    The payload [expr] is either a single token, or an application (that is, a
    list). A token is either a string or an antiquotation. Antiquotations are
    replaced by placeholder strings (see {!Antiquot}).
...
let ast_to_stream expressions =
  let strings =
    ...
    |> List.map @@ fun expr ->
       ...
       | Pexp_constant (Pconst_string (s, loc, _)) -> (s, loc.loc_start)
       | _ -> (Antiquot.create expr, expr.pexp_loc.loc_start)
  in
...
let markup_to_expr lang loc expr =
  ...
  let report loc error =
    ...
    | _ ->
        ...
        let message = Markup.Error.to_string error |> String.capitalize_ascii in
        Common.error loc "%s" message
  in
  let parser =
    Markup.parse_html ?context ~encoding:Markup.Encoding.utf_8 ~report
      input_stream
  in
  ...
  let rec assemble lang children =
    ...
    | Some (`Start_element ((ns, elt_name), attributes)) ->
        ...
        let node =
          Element.parse ~parent_lang:lang ~loc ~name ~attributes sub_children
        in
    ...
    | Some (`Comment s) ->
        ...
        assemble lang (node :: children)
    | Some (`Xml _ | `Doctype _ | `PI _) -> assemble lang children
  in

  let l = Element_content.filter_surrounding_whitespace @@ assemble lang [] in
...
let markup_to_expr_with_implementation lang modname loc expr =
  ...
  | Some modname ->
      ...
      res
  | _ -> markup_to_expr lang loc expr

let is_capitalized s =
  if String.length s < 0 then false
  else match s.[0] with 'A' .. 'Z' -> true | _ -> false
